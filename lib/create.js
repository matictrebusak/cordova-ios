...
exports.createProject = async (
  ...
  new ProjectCreator({
    ...
    options: {
      templatePath:
        opts.customTemplate || path.join(ROOT, "templates", "project"),
...
class ProjectCreator {
  ...
  provideProjectTemplate() {
    fs.cpSync(this.options.templatePath, this.project.path, {
      recursive: true,
    });
  ...
  updatePlatformConfigFile() {
    ...
    const configXmlPath = this.projectPath("__PROJECT_NAME__", "config.xml");

    fs.cpSync(defaultXmlPath, configXmlPath);

    const config = new ConfigParser(configXmlPath);
    xmlHelpers.mergeXml(
      this.options.rootConfig.doc.getroot(),
      config.doc.getroot(),
      "ios",
      /* clobber= */ true
    );
  ...
  configureCordovaLibPath() {
    ...
    const cdvLibXcodeAbsPath = path.join(
      cdvLibRealPath,
    ...
    const pbxprojPath = this.projectPath(
    ...
    transformFileContents(pbxprojPath, (contents) => {
      ...
      if (!line) {
        throw new Error(
          `Entry not found in project file for sub-project: ${cdvLibXcodePath}`
        );
      }
      ...
      if (!newLine.match("name")) {
        newLine = newLine.replace(
          "path = ",
          "name = CordovaLib.xcodeproj; path = "
        );
      }
