...
class Api {
  ...
  addPodSpecs(plugin, podSpecs, installOptions) {
    ...
    const minDeploymentTarget =
      this.getPlatformInfo().projectConfig.getPreference(
        "deployment-target",
        "ios"
      );
    ...
    const podsjsonFile = new PodsJson(
      path.join(project_dir, PodsJson.FILENAME)
    );
    ...
    if (podSpecs.length) {
      ...
      if (podfileFile.isDirty()) {
        ...
        return podfileFile
          ...
          .then(() =>
            podsjsonFile.setSwiftVersionForCocoaPodsLibraries(this.root)
          );
      } else {
